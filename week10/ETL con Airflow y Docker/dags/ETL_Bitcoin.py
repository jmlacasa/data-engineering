...
with open(dag_path+'/keys/'+"db.env",'r') as f:
    data_base= f.read()
with open(dag_path+'/keys/'+"user.env",'r') as f:
    user= f.read()
with open(dag_path+'/keys/'+"pwd.env",'r') as f:
    pwd= f.read()
with open(dag_path+'/keys/'+"host.env",'r') as f:
    url= f.read()
with open(dag_path+'/keys/'+"schema.env",'r') as f:
    schema= f.read()
...
def cargar_data(exec_date):
    ...
    insert_sql = f"INSERT INTO {redshift_conn['schema']}.{table_name} ({', '.join(columns)}) VALUES %s"
